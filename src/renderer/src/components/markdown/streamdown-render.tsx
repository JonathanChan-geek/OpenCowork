...
import { AdaptiveMarkdownTable, MarkdownTableStreamingContext } from './adaptive-markdown-table'
import {
  CollapsibleCodePre,
...
const BASE_STREAMDOWN_PLUGINS: PluginConfig = { cjk }
const STREAMDOWN_MATH_PLUGIN = createMathPlugin({ singleDollarTextMath: true })
  ...
  return (tree: RehypeSourceNode) => {
    const visit = (node: RehypeSourceNode) => {
      ...
      if (
        node.type === 'element' &&
        node.tagName &&
        MARKDOWN_SOURCE_BLOCK_TAGS.has(node.tagName) &&
        sourceLine
      ) {
        node.properties ??= {}
...
function buildStreamdownRehypePlugins(includeSourcePositions = false): StreamdownRehypePlugins {
  ...
  const tagNames = Array.from(
    new Set([...(sanitizeSchema.tagNames ?? []), ...Object.keys(STREAMDOWN_ALLOWED_HTML_TAGS)])
  )
  const sanitizeWithAllowedTags = [
    sanitizePlugin,
    {
      ...sanitizeSchema,
      tagNames,
      attributes: { ...sanitizeSchema.attributes, ...STREAMDOWN_ALLOWED_HTML_TAGS }
    }
...
function getInitialStreamdownPlugins(features: StreamdownFeatureFlags): PluginConfig {
  return features.math ? STREAMDOWN_MATH_BASE_PLUGINS : BASE_STREAMDOWN_PLUGINS
}
...
async function loadStreamdownPlugins(features: StreamdownFeatureFlags): Promise<PluginConfig> {
  ...
  if (!key) return BASE_STREAMDOWN_PLUGINS
  const cached = STREAMDOWN_PLUGIN_CACHE.get(key)
  if (cached) return cached
  const pending = STREAMDOWN_PLUGIN_PROMISE_CACHE.get(key)
  ...
  const promise = (async () => {
    ...
    if (features.math) plugins.math = STREAMDOWN_MATH_PLUGIN
    if (features.mermaid) {
      const { createMermaidPlugin } = await import('@streamdown/mermaid')
      plugins.mermaid = createMermaidPlugin({ config: { flowchart: { htmlLabels: false } } })
    }
  ...
  void promise.catch(() => STREAMDOWN_PLUGIN_PROMISE_CACHE.delete(key))
  return promise
}
...
function useStreamdownPlugins(content: string): PluginConfig {
  ...
  const [plugins, setPlugins] = React.useState<PluginConfig>(
    () => STREAMDOWN_PLUGIN_CACHE.get(pluginKey) ?? getInitialStreamdownPlugins(features)
  )

  React.useEffect(() => {
    let cancelled = false
    const cached = STREAMDOWN_PLUGIN_CACHE.get(pluginKey)
    if (cached) {
      setPlugins(cached)
      return
    }
    ...
    void loadStreamdownPlugins(features)
      ...
      .catch(() => {
        if (!cancelled) setPlugins(getInitialStreamdownPlugins(features))
      })
    return () => {
      cancelled = true
    }
...
function useStreamdownTranslations(): Partial<StreamdownTranslations> {
  ...
  return React.useMemo(
    () => ({
      close: t('streamMarkdown.controls.close'),
      copied: t('streamMarkdown.controls.copied'),
      copyCode: t('streamMarkdown.controls.copyCode'),
      copyLink: t('streamMarkdown.controls.copyLink'),
      downloadFile: t('streamMarkdown.controls.downloadFile'),
      downloadDiagram: t('streamMarkdown.controls.downloadDiagram'),
      exitFullscreen: t('streamMarkdown.controls.exitFullscreen'),
      viewFullscreen: t('streamMarkdown.controls.viewFullscreen'),
      openLink: t('streamMarkdown.controls.openLink')
    }),
    [t]
  )
}
...
function ThinkingSegmentBlock({
  ...
  React.useEffect(() => {
    ...
    if (wasActiveRef.current) setAccordionValue('')
  ...
  return (
    ...
    >
      <AccordionItem value="thinking" className="border-b-0">
        <AccordionTrigger className="group items-start gap-1.5 py-0 text-left no-underline hover:no-underline [&>svg:last-child]:hidden">
          <span className="min-w-0 flex-1">
            ...
            >
              <span className={cn('min-w-0', active && 'shimmer')}>
                {active ? t('streamMarkdown.thinking.active') : t('streamMarkdown.thinking.done')}
              </span>
            </span>
          </span>
          <ChevronDown
        ...
        <AccordionContent className="px-0 pb-0 pt-1.5 duration-[350ms] ease-in-out">
          <HTMLMarkdownRenderProvider
            className={cn(
              ...
              'text-[12px] leading-6 text-muted-foreground/84'
            )}
...
export const StreamdownRender = React.memo(function StreamdownRender({
  ...
  imageActions,
  artifactActions
}: StreamdownRenderProps): React.JSX.Element | null {
  ...
  } = useMarkdownCopy({ contentVersion: normalizedContent, renderVersion: plugins })
  const thinkingSegments = React.useMemo(
    () =>
      segments.filter(
        (segment): segment is Extract<RenderSegment, { type: 'thinking' }> =>
  ...
  const markdownSegments = React.useMemo(
    () =>
      segments.filter(
        (segment): segment is Extract<RenderSegment, { type: 'markdown' }> =>
  ...
  const activeMarkdownClassName =
    variant === 'thinking'
      ...
      : variant === 'user'
        ...
        : BASE_MARKDOWN_CLASSNAME
  const components =
    variant === 'thinking' ? THINKING_STREAMDOWN_COMPONENTS : DEFAULT_STREAMDOWN_COMPONENTS
  ...
  return (
    <div
      ...
      className={cn(
        'chat-font-content min-w-0 max-w-full overflow-hidden text-foreground [overflow-wrap:anywhere]',
