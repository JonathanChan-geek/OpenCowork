...
export const MarkdownTableAnalyzerOptionsContext = React.createContext<
  ColumnAnalyzerOptions | undefined
>(undefined)
...
export function AdaptiveMarkdownTable({
  children,
  className,
  node: _node,
  ...props
  ...
  const updateOverflowState = React.useCallback(() => {
    ...
    setHasHorizontalOverflow(scrollElement.scrollWidth - scrollElement.clientWidth > 2)
  }, [])
  ...
  return (
    <div className="markdown-table-shell" data-content-columns={contentColumnBucket}>
      <div
        ref={scrollRef}
        aria-describedby={hasHorizontalOverflow ? hintID : undefined}
        aria-label={t('streamMarkdown.scrollableTable')}
      ...
      <span id={hintID} className="sr-only">
        {t('streamMarkdown.scrollableTableHint')}
      </span>
    </div>
  )
}
  ...
  React.useEffect(() => {
    if (!streaming) {
      ...
      if (resolvedStaticTypes !== null) {
        setStreamingTypes((previous) =>
          areColumnTypesEqual(previous, resolvedStaticTypes) ? previous : resolvedStaticTypes
        )
      }
      return
    }
    ...
    if (analysisTimeoutRef.current !== null) return
    ...
    analysisTimeoutRef.current = window.setTimeout(() => {
      ...
      if (snapshot.rows.length < INITIAL_STREAMING_ROWS) return
      ...
      setStreamingTypes((previous) => {
        const merged = mergeColumnTypes(previous, candidateTypes)
        return areColumnTypesEqual(previous, merged) ? previous : merged
      })
...
function mergeColumnTypes(
  ...
  return candidateTypes.map((nextType, index) => {
    ...
    return previousType === undefined ? nextType : mergeColumnType(previousType, nextType)
  })
}

function areColumnTypesEqual(left: readonly ColumnType[], right: readonly ColumnType[]): boolean {
...
function getTableSnapshot(children: React.ReactNode): TableSnapshot {
  ...
  for (const section of sections) {
    ...
    for (const row of getChildElements(section.props.children)) {
      const values = getChildElements(row.props.children).map((cell) =>
        getReactNodeText(cell.props.children)
      )
      if (tagName === 'thead' && headers.length === 0) headers = values
...
function decorateTableChildren(
  children: React.ReactNode,
  columnTypes: readonly ColumnType[]
): React.ReactNode {
  return React.Children.map(children, (sectionNode) => {
    if (!React.isValidElement(sectionNode)) return sectionNode
    ...
    const rows = React.Children.map(section.props.children, (rowNode) => {
      ...
      const cells = React.Children.map(row.props.children, (cellNode, columnIndex) => {
        if (!React.isValidElement(cellNode)) return cellNode
...
function getElementTagName(element: ElementWithChildren): string {
  ...
  if (typeof element.type === 'string') return element.type.toLowerCase()
  const component = element.type as React.ComponentType
  return (component.displayName ?? component.name ?? '').toLowerCase().replace(/^markdown/, '')
}
