...
function renderMathInRawHTML(source: string): string {
  ...
  return source.replace(HTML_TOKEN_RE, (token) => {
    ...
    if (!tagName || !RAW_HTML_MATH_SKIP_TAGS.has(tagName)) return token

    if (/^<\//.test(token)) {
      const matchingIndex = skippedTags.lastIndexOf(tagName)
      if (matchingIndex >= 0) skippedTags.splice(matchingIndex, 1)
...
function visitRawHTMLNodes(node: HASTNode): void {
  if (node.type === 'raw' && node.value?.includes('$')) {
    node.value = renderMathInRawHTML(node.value)
  }
  for (const child of node.children ?? []) visitRawHTMLNodes(child)
}

export function renderRawHTMLMathRehypePlugin() {
  return (tree: HASTNode) => visitRawHTMLNodes(tree)
}
