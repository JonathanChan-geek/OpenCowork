...
const BARE_DOMAIN_URL_RE =
  /^(?:www\.)?(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+[A-Za-z]{2,63}(?::\d{2,5})?(?:[/?#][^\s]*)?$/
...
function normalizeURLProperty(node: HASTNode, property: 'href' | 'src'): void {
  if (!node.properties) return
  const normalized = normalizeBareURL(node.properties[property])
  if (normalized) node.properties[property] = normalized
}

function visitElementNodes(node: HASTNode): void {
  if (node.type === 'element') {
    if (node.tagName === 'a') normalizeURLProperty(node, 'href')
    else if (node.tagName === 'img') normalizeURLProperty(node, 'src')
  }

  for (const child of node.children ?? []) visitElementNodes(child)
}

export function normalizeBareURLRehypePlugin() {
  return (tree: HASTNode) => visitElementNodes(tree)
}
